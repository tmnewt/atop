...
class SinglePeriodClassic:
    ...
    def guide(self, hide_intro = False, 
        ...
        if hide_intro:
            ...
            print(''.join(intro_string.split('    ')))
                   
        if hide_what_is_an_option:
            pass
        else:
            self.__what_are_options_guide()
        
        print('\n******************')
        print(f'   Walkthrough')
        print('******************')
        
        start_problem_string = f'''
            Stating the problem:
        ...
        if self.position == 'Long':
            ...
            if self.optype == 'Call':
                ...
                print(''.join(long_call_sting.split('    ')))
                ...
                print(''.join(long_put_string.split('    ')))
            ...
            if self.optype == 'Call':
                ...
                print(''.join(short_call_string.split('    ')))
                ...
                print(''.join(short_put_string.split('    ')))
    ...
    def __what_are_options_guide(self):
        
        #whatare_options string chunk
        what_are_ops_string = f'''
            --------------------
             What are options?  
            --------------------
        ...
        print(''.join(what_are_ops_string.split('    ')))
    ...
    def __binomial_reminder(self):
        reminder_string = f'''
            ...
            Since this is a single period option where {self.underlying_name} starts at 
             {self.underlying_value} and either moves up to {self.up_value} or down to {self.down_value} 
             (and there are no other values {self.underlying_name} can take) 
             we only need to calculate the potential payoffs for 2 states. 
             Now given this is a {self.position} {self.optype} the potential payoff values 
             can be  described using the following mathematical relationship:'''
        return ''.join(reminder_string.split('    '))

    
    # function to keep the guide from being bloated.
    def __guide_payoff(self):
        if self.position == 'Long':
            if self.optype == 'Call':
                temp_string = f''' 
    ...
    # end of __guide_payoff()


    def __guide_hedge(self):
        hedge_string = f'''
            The next step is calculating the hedge ratio. It is a rather simple 
            calculation.

            hedge_ratio = (up_payoff - down_payoff)/(up_value - down_value) 

              Plug in the values we have so far and find that:

            hedge_ratio = ({self.up_payoff:.2f} - {self.down_payoff:.2f})/({self.up_value:.2f} - {self.down_value:.2f}) = {(
                    (self.up_payoff - self.down_payoff)/(self.up_value - self.down_value)
                ):.2f}'''     
        return ''.join(hedge_string.split('    '))
        #end of __guide_hedge()
